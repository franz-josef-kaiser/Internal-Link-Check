...
class ilcInit
{
	 ...
	 * Used to determine in the extended WP_List_Table class
	...
	public static function load_textdomain()
	{
		...
		load_textdomain( $domain, "{$dir}lang/{$domain}-{$locale}.mo" );
	}
	 ...
	 * @return string $data
	 */
	private static function get_plugin_data( $value = 'Version', $mark_up = true )
	{
	...
	public function markup( $results )
	{
		...
		if (
			! $this->args[ 'container' ]
			OR 'li' === $this->args[ 'element' ]
		)
			$output = "<%CONTAINER%%CONTAINER_CLASS%>{$output}</%CONTAINER%>";

		return $this->markup_filter( $output );
	}
	...
	public function update_message( $plugin_data, $r )
	{
		...
		return print $output .= sprintf(
			__( 'You can also check the nightly builds of %1$sour development repository%2$s on GitHub. If you got ideas, feature request or want to help with pull requests, please feel free to do so on GitHub.%3$s', 'ilc' ),
